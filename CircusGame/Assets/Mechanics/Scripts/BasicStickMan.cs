...
public partial class BasicStickMan : MonoBehaviour
{
    ...
    private void Update() {
        ...
        {
            Vector2 forceDir = new Vector2(0, transform.position.y + 100);
            gameObject.GetComponent<Rigidbody2D>().AddForce(forceDir * 100);
        }
    ...
    public void Walk() {
        RaycastHit2D hit = dollRaycast.RaycastClosest(transform.position, Vector2.down,3);
        if(hit)
        {
